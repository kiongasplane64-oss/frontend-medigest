...
export default function ExpensesManager() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 pb-10">
      ...
      <div className="flex-1 space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black text-slate-800 tracking-tight text-center md:text-left">Gestion des Dépenses</h1>
              <p className="text-slate-500 font-medium italic text-center md:text-left text-sm">Flux financiers en temps réel.</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-8 rounded-4xl border border-slate-100 shadow-sm flex items-center gap-6">
            ...
            <div>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-1">Total Sorties (DB)</p>
          ...
          <div className="bg-white p-8 rounded-4xl border border-slate-100 shadow-sm flex items-center gap-6">
            <div className="w-16 h-16 bg-slate-50 text-slate-500 rounded-3xl flex items-center justify-center shadow-inner font-black">
              <Wallet size={32}/>
            </div>
            <div>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-1">Transactions validées</p>
          ...
          <div className="p-6 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/30">
            ...
            <div className="relative w-full md:w-auto">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 bg-white border border-slate-100 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-red-500" 
              />
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-slate-50">
                {expenses?.filter(e => (e.description || '').toLowerCase().includes(searchTerm.toLowerCase())).map((expense) => (
                  <tr key={expense.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-8 py-4 text-xs text-slate-500 font-medium italic">{expense.description || 'N/A'}</td>
            ...
            {(!expenses || expenses.length === 0) && (
              <div className="py-24 text-center flex flex-col items-center">
                ...
                <p className="text-slate-400 font-bold italic text-sm">Aucun enregistrement en base de données.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* SIDEBAR AUDIT (JOURNAL DB) */}
      <div className="w-full lg:w-80 space-y-6">
        <div className="bg-slate-900 rounded-4xl p-6 shadow-2xl shadow-slate-200 sticky top-4">
          ...
          <div className="space-y-6">
            {loadingLogs ? <div className="text-slate-600 text-[10px] animate-pulse">Synchronisation logs...</div> :
              ...
              <div key={log.id} className="relative pl-6 border-l-2 border-slate-800 group">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2 text-slate-400">
                    <User size={12} />
                    <span className="text-[10px] font-black uppercase text-slate-300 truncate">{log.user}</span>
                  </div>
                  <p className="text-xs text-slate-400 font-medium leading-snug">
                    {log.action} <span className="text-slate-200 font-bold">{log.target}</span>
                  </p>
