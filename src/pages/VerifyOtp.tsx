...
import { 
  ArrowLeft, RefreshCw, Smartphone, AlertCircle, 
  CheckCircle2, LogIn, Mail, MessageCircle, Edit2 
} from 'lucide-react'; 
...
export default function VerifyOtp() {
  ...
  // Correction 6133: On utilise uniquement le mail venant de l'URL
  ...
  useEffect(() => {
    ...
    if (checkUser?.actif === true) navigate('/dashboard');
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 border border-slate-100 relative overflow-hidden">
        ...
        {isSuccess && (
          <div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center animate-in fade-in">
            <div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-4"><CheckCircle2 size={48} /></div>
          ...
          {!showResendOptions ? (
            ...
            <div className="space-y-3 animate-in slide-in-from-bottom-2">
              ...
              <div className="flex flex-col gap-2">
                <button onClick={() => handleResend('sms')} className="flex items-center justify-between p-3 bg-slate-50 hover:bg-blue-50 rounded-xl text-sm font-medium transition-colors border border-transparent hover:border-blue-100">
                  <div className="flex items-center gap-3 text-slate-700"><Smartphone size={18} className="text-blue-500"/> Par SMS</div>
                  <ArrowLeft size={14} className="rotate-180 text-slate-300" />
                </button>
                <button onClick={() => handleResend('whatsapp')} className="flex items-center justify-between p-3 bg-slate-50 hover:bg-green-50 rounded-xl text-sm font-medium transition-colors border border-transparent hover:border-green-100">
                  <div className="flex items-center gap-3 text-slate-700"><MessageCircle size={18} className="text-green-500"/> Par WhatsApp</div>
                  <ArrowLeft size={14} className="rotate-180 text-slate-300" />
                </button>
                <button onClick={() => handleResend('email')} className="flex items-center justify-between p-3 bg-slate-50 hover:bg-purple-50 rounded-xl text-sm font-medium transition-colors border border-transparent hover:border-purple-100">
                  <div className="flex items-center gap-3 text-slate-700"><Mail size={18} className="text-purple-500"/> Par Email</div>
                  <ArrowLeft size={14} className="rotate-180 text-slate-300" />
                </button>
              </div>
              <button onClick={() => setShowResendOptions(false)} className="text-xs text-slate-400 mt-2 font-bold">ANNULER</button>
            </div>
          )}
        </div>

        <button onClick={() => navigate('/login')} className="mt-8 flex items-center justify-center gap-2 w-full text-sm text-slate-500 font-bold hover:text-blue-600 transition-colors">
